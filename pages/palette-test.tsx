...
export default function PaletteTest() {
  ...
  const shadeNumbers: PaletteShade[] = [50, 100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 950];
    ...
    <div style={{ padding: '40px', fontFamily: 'monospace', color: 'white' }}>
      ...
      <div style={{ marginTop: '40px' }}>
        ...
        <div style={{ fontSize: '12px', marginBottom: '10px' }}>
          <strong>Gamut:</strong> {gamut} | <strong>Shades:</strong> {palette.length}
        </div>
        {palette.map(({ shade, luminosity, color }) => (
          <div key={shade} style={{ display: 'flex', gap: '20px', marginBottom: '8px', alignItems: 'center' }}>
            ...
            />
            <div style={{ flex: 1, fontSize: '13px', fontFamily: 'Monaco, monospace' }}>
              {color}
            </div>
...
{`{
  ...
  gamut: '${gamut}'
        ...
        <div style={{ marginTop: '15px', fontSize: '14px', lineHeight: '1.6' }}>
          <p><strong>lightness</strong> (0-1): The lightness of the key color</p>
          <p><strong>chroma</strong> (0-0.4): The colorfulness/saturation</p>
          <p><strong>hue</strong> (0-360): The color angle (276° = purple-blue)</p>
          <p><strong>lowerCp</strong>: Dark curve control (1 = straight)</p>
          <p><strong>upperCp</strong>: Light curve control (1 = straight)</p>
          <p><strong>torsion</strong>: Prevents undesirable color shifts (e.g., "mustard problem")</p>
          <p><strong>gamut</strong>: Color space (p3 = Display P3, srgb = standard RGB)</p>
        </div>
      </div>
    </div>
  );
